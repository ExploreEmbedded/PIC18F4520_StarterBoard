...
{
    uint8_t gie_value;
    while(util_GetBitStatus(EECON1,SBIT_WR)==1);           // check the WR bit to see if a WR is in progress
    ...
    gie_value = GIE;
    GIE = 0;
	util_BitSet(EECON1,SBIT_WREN);  // Set the WREN bit to enable eeprom operation.
    EECON2=0x55;               //Execute the special instruction sequence
	EECON2=0xaa;               //Refer the datasheet for more info
	util_BitSet(EECON1,SBIT_WR);                      // Set the WR bit to trigger the eeprom write operation.
    util_BitClear(EECON1,SBIT_WREN); 
    GIE = gie_value;
}
...
uint8_t EEPROM_ReadByte(uint16_t v_eepromAddress_u16)
{
   while((util_GetBitStatus(EECON1,SBIT_WR)==1) || (util_GetBitStatus(EECON1,SBIT_RD)==1)); 
 	EEADR=v_eepromAddress_u16;   // Write the address to EEADR.
                                 // Make sure that the address is not larger than the memory size 
	util_BitSet(EECON1,SBIT_RD); // Set the WR bit to trigger the eeprom read operation.
